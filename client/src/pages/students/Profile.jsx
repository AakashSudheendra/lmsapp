...
import { useLoadUserQuery, useUpdateUserMutation } from "@/feautures/api/authApi";
...
const Profile = () => {
  ...
  },[updateUserData,isSuccess,error])
    ...
    <div className="max-w-4xl mx-auto my-24 px-4">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8 my-5">
        <div className="flex flex-col items-center">
          <Avatar className="rounded-lg h-24 w-24 md:h-32 md:w-32 mb-4">
            <AvatarImage
        ...
        <div>
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button size="sm" className="mt-2 hover:bg-gray-700">
                Edit Profile
              </Button>
            </DialogTrigger>
            <DialogContent>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 my-5">
          {user?.enrolledCourses.length === 0 ? (
            <h1>You haven't enrolled Cources</h1>
          ) : (
            user?.enrolledCourses.map((cource) => <Cource cource={cource} />)
          )}
        </div>
      </div>
    </div>
